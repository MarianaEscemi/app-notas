...
export const signup = async (req, res) => {
  ...
  if (errors.length > 0) {
    return res.render("auth/signup", {
      errors,
      name, /* se envian de nuevo todo al front asi corrije */
      email, /* se envian de nuevo todo al front asi corrije */
      password, /* se envian de nuevo todo al front asi corrije */ 
      confirm_password,
    });
  }
...
export const logout = async (req, res, next) => {
  await req.logout((err) => {
    ...
    req.flash("success_msg", "Estás deslogueado ahora.");
    res.redirect("/auth/signin");
  });
};
